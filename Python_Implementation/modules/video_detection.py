...
from modules.darknet.preprocess import letterbox_image
...
def start_video(video_path, configuration, sio):
    ...
    if tots > 0:
        sio.emit('tots', {"tots": tots, "width": frame_width, "height": frame_height})

    frames_processed = 0
    global output, command, frame_number
    frame_number = 0
    output = None
    command = 'play'
    static_back = None
    ...
    while True:
        try:
            ...
            if ret:
                img, orig_im, dim = prep_image(im, inp_dim)
                ...
                if is_motion and frame_number % 3 == 0:
                    ...
                    try:
                        ...
                        sio.emit('progress',
                                 {"valid": False,
                                  'message': 'CUDA, out of memory error. Please reduce input resolution'})
                        return
