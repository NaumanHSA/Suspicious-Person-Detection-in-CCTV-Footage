...
import numpy as np
import cv2
from modules.darknet.util import convert2cpu as cpu
from modules.darknet.util import predict_transform
...
def parse_cfg(cfgfile):
    ...
    lines = [x for x in lines if x[0] != '#']  #get rid of uselesss lines
...
def create_modules(blocks):
    ...
    for x in blocks:
        ...
        if (x["type"] == "convolutional"):
            ...
            if padding:
                #spacify number of layers for padding
                pad = (kernel_size - 1) // 2
            else:
                pad = 0
